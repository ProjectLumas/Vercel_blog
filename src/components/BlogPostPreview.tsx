  ...
  return (
    <div className="break-words">
      ...
      <div className="grid grid-cols-1 gap-3 md:col-span-2 mt-4">
        ...
        <div className="prose lg:prose-lg italic tracking-tighter text-muted-foreground">{formatDate(new Date(publishedAt || updatedAt), "dd MMMM yyyy")}</div>
        <div className="prose lg:prose-lg leading-relaxed md:text-lg line-clamp-4 text-muted-foreground">{Description}</div>
        <div className="text-sm text-muted-foreground">{tags?.data.map((tag) => (<div key={tag.id} className="mr-2 inline-block"><Link href={`/tag/${tag.attributes.Slug}`}>#{tag.attributes.Name}</Link></div>))}</div>
      </div>
    </div>
  );
};
